...
LPTSTR CBAdoRecordset::FieldsCount()
{
	try
	{
		return (TCHAR *)((_bstr_t)m_pRst -> GetFields() -> Count);
		m_ErrStr[0] = TEXT('\0');		// 清空错误信息字符串为 ""
	}
	catch(_com_error &e)
	{
...
LPTSTR CBAdoRecordset::FieldName( long index )
{
	try
	{
		return (TCHAR *)(_bstr_t)m_pRst -> GetFields() -> GetItem(index) -> Name;
		m_ErrStr[0] = TEXT('\0');		// 清空错误信息字符串为 ""
	}
	catch(_com_error &e)
	{
